...
for noise_idx, train_snr in enumerate(args.snr_range):
    ...
    config.data.noise_std      = 10 ** (-config.data.train_snr / 10.) 
    ...
    H_val_complex = torch.tensor(H[:7500, :, :]).detach().numpy()#Pick up NT random users from 100.
    # H_val_complex = torch.tensor(H[:, :, :]).detach().numpy()#Pick up NT random users from 100.

    

    # Create locals
    dataset = Channels(train_seed, config,  H = H_val_complex, norm=[0, 1])
